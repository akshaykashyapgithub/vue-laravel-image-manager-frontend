          ...
          <div class="flex items-center">
            ...
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                ...
                :to="item.to" 
                 class="nav-link"
                :class="[$route.name === item.to.name ? 'bg-gray-950/50 text-white' 
            ...
            <div class="ml-4 flex items-center md:ml-6">
              ...
              <Menu as="div" class="relative ml-3">
                <MenuButton class="relative flex max-w-xs items-center rounded-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
                  ...
                  <img class="size-8 rounded-full outline -outline-offset-1 outline-white/10" src="https://img.freepik.com/premium-vector/person-with-blue-shirt-that-says-name-person_1029948-7040.jpg?semt=ais_hybrid&w=740&q=80" alt="" />
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
                  <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-gray-800 py-1 outline-1 -outline-offset-1 outline-white/10">
                    <MenuItem>
                      <button @click="Logout" :class="['block px-4 py-2 text-sm text-gray-300']">Sign out</button>
                    </MenuItem>
                  </MenuItems>
                </transition>
              </Menu>
      ...
      <DisclosurePanel class="md:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
          ...
          :key="item.name"
           as="a" 
           :to="item.to" 
           :class="[$route === item.to.name ? 'bg-gray-950/50 text-white' : 'text-gray-300 hover:bg-white/5 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']" 
           :aria-current="$route === item.to.name  ? 'page' : undefined">{{ item.name }}</DisclosureButton>
        </div>
        <div class="border-t border-white/10 pt-4 pb-3">
          ...
          <div class="mt-3 space-y-1 px-2">
            ...
            class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-white/5 hover:text-white">Sign Out</DisclosureButton>
          </div>
        </div>
      </DisclosurePanel>
    </Disclosure>   
...
const navigation = [
  { name: 'Upload Images', to: { name: 'Home' }},
